...
function CameraScanner({ mode, onDecode, onClose }: CameraScannerProps) {
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-black/80">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white text-sm font-medium">
            {mode === 'location' ? 'Ler QR Code da localização' : 'Ler código de barras do produto'}
          </span>
        </div>
        <button
          onClick={onClose}
...
export default function ScanMobile() {
  ...
  }, [scanMode, showFeedback, activeLocation]);
